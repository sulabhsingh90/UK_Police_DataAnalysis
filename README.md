# Analysis of UK Police Data using Apache spark (scala)



> Dataset used in this example is from [**UK POLICE DEPARTMENT**][1]
...
2. Find out the frequency of each crime (Number of crimes committed)

3. Find out the percentage of cases where accused is not guilty in the outcome
...
> Have you noticed that we have mentioned Crime Type column with **col** in groupBy, but in **withColumnRenamed** method we are simply passing column name as a string. This is because Spark will compile your expressions or functions or SQL logics down to an underlying plan before actually executing, so every statement will run exactly the same beneath the skin.
...
* Find out the percentage of cases where accused is not guilty in the outcome
...
> Collect is a very expensive operation that can crash your driver program when dealing with large dataset. Its also not recommended as it will process data one by one basis instead of running in parallel
