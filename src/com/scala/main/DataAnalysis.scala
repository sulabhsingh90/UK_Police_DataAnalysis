...
object DataAnalysis {
	...
	def main(args:Array[String]){
		val spark=SparkSession.builder().appName("DataAnalysis").master("local[*]").getOrCreate()
				...
				val topCrime=ukPoliceDF.groupBy("crime type")
				.count().withColumnRenamed("count", "total")
				.orderBy(desc("total")).collect().foreach(println)
				...
				.groupBy("crime type").agg(count("*").alias("total"),
						sum(when(col("Last outcome category")==="Defendant found not guilty",lit(1)).otherwise(lit(0))
								).alias("notguilty")).withColumn("notguiltypercent", bround(col("notguilty")/col("total")*lit(100),2)).orderBy(desc("notguilty"),desc("total")).collect().foreach(println)
